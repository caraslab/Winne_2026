function plotAblationDuringPLFAs(datafile,cutoff,clrs,figsavepath,varargin)
...
measCols = arrayfun(@(x) sprintf('FArateDay%d', x), dayvec, 'UniformOutput', false);
...
for i = 1:numel(treatments)
    ...
    %Extract FArates
    FArates = tblFilt{:,measCols};

    numSubj = size(FArates,1);
...
fig1 = gcf;
if nargin > 4
    figname = 'ablation_afterPL_FArates.pdf';
else
    figname = 'ablation_duringPL_FArates.pdf';
end
