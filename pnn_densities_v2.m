function [TposSubj, TnegSubj] = pnn_densities_v2(pvposFile, pvnegFile, outPrefix, groups, colors)
% [TposSubj, TnegSubj] = pnn_densities_v2(pvposFile, pvnegFile, outPrefix, groups, colors)
%
% Description:
...
% Written by ML Caras June 2025
    ...
    area = 0.497185*0.930*0.012; %mm^3 L4-L6 over 12 um depth span
...
function plotBars(Tsubj, groups, colors, ttl)
    ...
    % %Box plot
    % boxplot(Tsubj.PNNDensity,Tsubj.Group,'colors',colors);
    % hl =findobj('LineStyle','--'); set(hl, 'LineStyle','-'); %solid lines
    % ax = gca;
    % bh = ax.Children.Children;
    % set(bh,'Linewidth',2) %inrcrease linewidth
    % allTags = get(bh, 'tag');
    % isCap = contains(allTags,'Adjacent'); 
    % delete(bh(isCap))
    % hold on;
